const AT_APIKey = 'b2f3f9310f5e4076b779352d107d9e94';
	...
	aucklandMap.on('singleclick', function(event){
		...
		if(aucklandMap.hasFeatureAtPixel(event.pixel)){
			...
			if(feature.markerType === 'Bus'){ //set the bus popup info
				...
				document.getElementById('busPopupContent').innerHTML = `${popupRouteNum} ${popupRouteName} <br> Occupancy: ${popupOccupancy} <br><br>`;
...
function getStopById(stops, busLat, busLong){ //Estimate the next stop in {stops} using time, then call the API to get info about that stop
	...
	xhr.onreadystatechange = () =>{
		if(xhr.readyState === XMLHttpRequest.DONE){
			...
			document.getElementById('busPopupContent').innerHTML += `Next stop: ${stopCode} - ${stopName} <br> ETA: ${nextStopTime} (Current time: ${now.getHours()}:${now.getMinutes()}:${now.getSeconds()})`; //add next stop information to popup
...
const formatTimetable = (res) =>{
	...
	for(let i = 0; i < res.response.length; i++){
		...
		if(now <= stopTime && pickupType != 1){ //don't display stops whose times that have already passed
			let indexOf_to = routeName.indexOf(' To ') + 1;
			let indexOf_via = routeName.indexOf(' Via ');
...
const toggleAutoUpdate = () =>{
	let updateTime = 30; //time reset to 30 seconds each time button is pressed
